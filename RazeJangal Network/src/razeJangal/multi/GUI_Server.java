...
public class GUI_Server {
	...
	private class PrivateMessage extends JButton implements MouseListener {
		...
		public void mouseClicked(MouseEvent e) {
			if (firstAct==true) {
				firstAct = false;
				GUI_Server.this.print(pm);
				refreshAll();
				this.setText("Hide!");
			} else {
				GUI_Server.this.print(":)");
				this.setVisible(false);
			}
		}
		public void mousePressed(MouseEvent e) {			}
	...
	private class changeGoalTreasure extends JButton implements MouseListener {
		...
		public void clicked() {
			...
			sendAllClients("GoalTreasure#" + game.treasures.goalTreasure);
			refreshAll();
			GUI_Server.this.print("Round number ROUNDNUMBER, this round's goal treasure has changed to GOALTREASURE");
			throwDices();
			refreshAll();			
		}
		public void mouseClicked(MouseEvent e) {
	...
	private Scanner[] sc;
	private ServerRecvThread[] st;
	
	
	public GUI_Server(Socket[] socket, String mapPath) {
		
		System.out.println("Server: i am started");
		
		numberOfPlayers = socket.length;
		ps = new PrintStream[numberOfPlayers];
		sc = new Scanner[numberOfPlayers];
		st = new ServerRecvThread[numberOfPlayers];
		
		for (int i = 0; i < socket.length; i++) {
			try {
				ps[i] = new PrintStream(socket[i].getOutputStream());// writing to socket
				sc[i] = new Scanner(socket[i].getInputStream()); // reading from socket
				st[i] = new ServerRecvThread(sc[i], i);
				st[i].start();
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
		...
		game = new Game(numberOfPlayers, mapPath);
		for (int i = 0; i < game.player.length; i++) {
			game.player[i].setName("PLAYER" + new Random().nextInt(100));
		}
		
		sendAllClients("Start#");
		...
		// send treasures to players
		String treasuresString = "";
		for (int i = 0; i < game.treasures.treasureIndex.length; i++) {
			treasuresString = treasuresString + "\n" + "Treasures#" + i + "#" + game.treasures.treasureIndex[i] + "#" + game.treasures.treasureName[i] + "#";
		}
		sendAllClients(treasuresString);
		
		// ask clients to send their name
		sendAllClients("SendYourName#");
		...
		new Thread() {
			public void run() {
				...
				temp = "Names#";
				for (int i = 0; i < game.player.length; i++) {
					temp = temp + game.player[i].getName() + "#";					
				}
				sendAllClients(temp);
			...
			public void run() {
				...
				while (flag) {
					...
					for (int i = 0; i < numberOfPlayers; i++) {
						pm = pm + game.getPlaces()[i] + "#";
					}
					sendAllClients("Turn#" + game.getTurn()+ "\n" +
					"SameDice#" + sameDice + "\n" +
		...
		for (int i = 0; i < game.treasures.treasureNumbers(); i++)
			if (game.getPlaces()[game.getTurn()] == game.treasures
				...
				*/
				...
				sendAllClients("PublicPM#" + "Cell " + game.player[game.getTurn()].getPlace() + "'s treasure seen by player "+ game.player[game.getTurn()].getName());
				sendClient("PrivatePM#" + "Cell " + game.player[game.getTurn()].getPlace() + "'s treasure is "
						+ game.treasures.index2Treasure(game.player[game.getTurn()].getPlace()), game.getTurn());
			}
	...
	private void clickedOnPartner(int index) {
		...
		if (sameDice==true) {
			...
			if (game.treasures.getGoal().equals(game.treasures.index2Treasure(index))) {
				...
				game.treasures.changeGoal();
				sendAllClients("GoalTreasure#" + game.treasures.treasure2Index(game.treasures.getGoal()));
				print("Round number ROUNDNUMBER started, this round's goal treasure is GOALTREASURE");
	...
	private void throwDices() {
		...
		secondDice = 0;
		dice = Game.diceThrow();
		sendAllClients("Dices#" + dice[0] + dice[1]);
	...
	private void sendClient (String request, int playerNum) {
		while (!openMonitor) {
			;
		}
		ps[playerNum].println(request);
		try {
			Thread.sleep(200);
		} catch (InterruptedException e) {
			// TODO Auto-generated catch block
	...
	private void sendAllClients (String request) {
		for (int i = 0; i < ps.length; i++) {
			sendClient(request, i);
		}
	}
	...
	class ServerRecvThread extends Thread {
		...
		public ServerRecvThread(Scanner sc, int playerNum) {
			this.sc = sc;
		...
		public void run() {
			try {
				...
				while (true) {
					...
					//System.out.println("Server: Recived form player" + playerNum + " : " + response);
					// TODO handle client requests
					String responseBAK = response;
					if (response.startsWith("NAME#")) {
						response = StringMethods.replaceWord(response, "NAME#", "");
						game.player[playerNum].setName(response);
					} else if (response.startsWith("CHAT#")) {
						response = StringMethods.replaceWord(response, "CHAT#", "");
						sendAllClients("Chat#" + playerNum + "#" + response + "#");
					} else if (response.startsWith("NUMBEROFPLAYERS#")) {
						sendClient("NumberOfPlayers#" + game.player.length, playerNum);
						sendAllClients("NumberOfPlayers#" + game.player.length);
					} else if (game.getTurn() == playerNum) {
						if (response.startsWith("SELECT#")) {
							response = StringMethods.replaceWord(response, "SELECT#", "");
							int index = Integer.parseInt(response);
							sendAllClients(responseBAK);
							clickedOn(index);
						...
						} else if (response.startsWith("CHANGETREASURE#")) {
							responseBAK = "GoalTreasure#" + game.treasures.goalTreasure;
							sendAllClients(responseBAK);
							optionsBox.goalTreasure.clicked();
						} else {
							sendAllClients(responseBAK);
							System.out.println("Server Doesn't Understand Client!");
						}
						//sendAllClients(responseBAK);
					} else {
						System.out.println("Server Doesn't Understand Client!");
